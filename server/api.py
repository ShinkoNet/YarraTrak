...
# cache departures by the route bits
...
async def text_query(request: AgentRequest):
    ...
    if not llm_key:
        raise HTTPException(status_code=401, detail="legacy provider API key required. Provide llm_api_key in request body.")
...
async def websocket_endpoint(websocket: WebSocket, buttons: str = Query(None), llm_api_key: str = Query(None)):
    ...
    llm_api_key: Optional legacy provider API key for agent queries (BYOK).
    ...
    if buttons:
        try:
            ...
            if parsed_buttons:
                ...
                # initial favourite fetches run in parallel
            ...
            if msg_type == "query":
                ...
                if not resolved_key:
                    await websocket.send_json({
                        "type": "error",
                        "id": msg_id,
                        "error": "legacy provider API key required for agent queries. Configure in settings."
                    })
                    continue
