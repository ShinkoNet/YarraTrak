"""
Agent Engine - Pure Tool-Call Architecture (legacy provider OpenRouter)
...
import logging
import hashlib
from anthropic import Asynclegacy provider, APITimeoutError, APIConnectionError, BadRequestError
...
# legacy provider uses "input_schema" instead of "parameters"

def _groq_to_anthropic_tool(tool_def: dict) -> dict:
    """Convert a Groq-format tool definition to legacy provider format."""
...
def _get_client(llm_api_key: str) -> Asynclegacy provider:
    """Get an legacy provider client using the user-provided key (BYOK)."""
    return Asynclegacy provider(api_key=llm_api_key)
...
async def run_worker(query: str, session_id: str, prefetched_context: str = "", llm_api_key: str | None = None) -> dict:
    ...
    Args:
        ...
        llm_api_key: Optional per-request legacy provider API key (BYOK)
    """
    ...
    # Build messages (legacy provider format: system is separate, not in messages)
    messages = []
    ...
    for turn in range(10):
        try:
            ...
            error_str = str(e)
            logger.warning(f"[Turn {turn}] legacy provider BadRequestError: {error_str}")
        ...
        # process response content blocks legacy provider returns content as a list of blocks (text and/or tool_use)
        ...
        for block in tool_use_blocks:
            ...
            # Add tool result in legacy provider format
            tool_results.append({
                "type": "tool_result",
