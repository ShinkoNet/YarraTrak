...
// on aplite we don't draw rings behind the large countdown text - the 1-bit dotted pattern can otherwise compete with the
...
// stippled "grey" ring: plot single pixels around the circumference instead of a continuous stroke
...
static void ripple_update_proc(Layer *layer, GContext *ctx) {
  ...
  // dark theme: solid indigo rings on black read well
