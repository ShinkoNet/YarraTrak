...
static void set_card_text(const char *title, const char *body) {
  ...
  if (s_body_layer) {
    ...
    // measure text with enough height first
...
static void choices_select(MenuLayer *menu_layer, MenuIndex *cell_index, void *context) {
  ...
  // pop the picker and keep the card alive
  if (s_choices_window) {
...
// clarification payload is split in-place
...
void query_window_show_progress(const char *label) {
  if (!s_card_window) return;
  // preserve the "you: <transcription>" body, just retitle
...
bool query_window_is_open(void) {
  return s_card_window != NULL;
       ...
       // keep clarify state out of old heaps
