...
function runStealthQuery(buttonIndex) {
    ...
    function formatCountdown() {
        ...
        // parse utc time - server sends iso format like "2025-12-14t07:59:00+00:00" some js engines don't parse +00:00 correctly, so
        var timeStr = dep.departure_time.replace(/\+00:00$/, 'Z');
        var depTime = new Date(timeStr);
        var now = new Date();
    ...
    if (dep && (dep.departure_time || dep.minutes !== null)) {
        ...
        // calculate vibration pattern locally (instant feedback, no network call) use fresh minutes calculated from departure_time,
        var pattern = null;
