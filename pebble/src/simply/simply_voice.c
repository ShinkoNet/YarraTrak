...
static void handle_voice_start_packet(Simply *simply, Packet *data) {
  // send an immediate response if there's already a dictation session in progress status 64 = sessionalreadyinprogress
  if (s_voice->in_progress) {
    send_voice_data(64, "");
    return;
  }

  // otherwise, start the timer as soon as possible (we start a timer so we can return true as quickly as possible)
