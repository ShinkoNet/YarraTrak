...
// calculate vibration pattern locally (same logic as pebble app) encodes minutes as haptic pattern: hours=1000ms, tens=500ms,
...
function connectWebSocket() {
    ...
    ws.onopen = () => {
        ...
        updateWsStatus();
        // note: server will push favourite_update immediately if buttons were in url still call sendfavouritesubscription for
...
async function sendFavourite(id) {
    ...
    try {
        ...
        // calculate vibration pattern from message (extract minutes) message format: "x min" or "arriving now" or "now"
...
async function closeConversation() {
    ...
    clearClarifyOptions();
    updateStatus("Ready");
    // note: session id persists for query history (speculative fetch) only the ui/conversation context is cleared
}
